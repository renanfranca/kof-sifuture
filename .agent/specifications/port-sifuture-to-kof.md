...
- For the ship sprite in this port, `Middle2.png` MUST appear only while ArrowRight is the effective horizontal direction. Releasing ArrowRight MUST restore `Middle.png` immediately or on the next fixed step, including when only vertical movement remains. Opposite horizontal keys use the last pressed direction. This is an explicit exception to the historical sprite behavior, which retains the jet frame after release.
